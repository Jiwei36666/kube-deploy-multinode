...
```console
$ git clone https://github.com/Jiwei36666/kube-deploy-multinode
$ cd kube-deploy-multinode
$ ./master.sh
```
...
## Deploy container from master node dashboard

After master node is done to startup, click [http:$masterip:8080/ui](http:127.0.0.1:8080/ui) for dashboard.

Create file "hello-world.yaml" and copy below content into the file. On the dashboard window, click "Create" button on the top right, then selete option "Upload a YAML or JSON file", chose the file "hello-world.yaml", click "UPLOAD" button to start a container on the ARM based slave node.
...
spec:
  ...
  containers:
    - name: arm-hello
      image: ericvh/arm64-ubuntu:latest
      imagePullPolicy: IfNotPresent
      securityContext:
        privileged: true
      command: ['/bin/bash','-c']
      args: ['while true;do echo "$i: $(date)"; i=$((i+1)); sleep 10;done']  
  nodeSelector:
